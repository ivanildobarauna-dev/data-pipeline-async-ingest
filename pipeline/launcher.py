...
from pipeline.config.common import default_timestamp_formated
from pipeline.config.beam_config import BeamConfig
from pipeline.config.common import EnvSetup, Constants
...
def print_count(element):
    print(
        f"{default_timestamp_formated()} Qtde Messages Processed: {len(element[0][1])}"
    )


def run():
    pipeline_options = PipelineOptions.from_dictionary(
        BeamConfig(EnvSetup.PROD).get_pipeline_options()
    )
